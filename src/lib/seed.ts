...
function defaultMetrics(accountId: string, scale = 1): AccountMetrics {
  ...
  return {
    ...
    kpis: [
      { label: "Vistas", value: `${(3.8 * scale).toFixed(1)}M`, delta: "+31%", detail: "Reels + stories en 30d", color: "#58e6ff" },
      { label: "Reach", value: `${(1.42 * scale).toFixed(2)}M`, delta: "+18%", detail: "Cuentas alcanzadas", color: "#d8ff63" },
      ...
      { label: "Comentarios", value: `${(18.2 * scale).toFixed(1)}K`, delta: "+44%", detail: "Señal de conversación", color: "#ffc857" },
      ...
      { label: "Saves", value: `${(31.6 * scale).toFixed(1)}K`, delta: "+26%", detail: "Contenido de alta utilidad", color: "#58e6ff" },
      { label: "Shares", value: `${(12.9 * scale).toFixed(1)}K`, delta: "+19%", detail: "Contenido reenviable", color: "#d8ff63" },
      { label: "DMs", value: `${s(2184)}`, delta: "+38%", detail: "Conversaciones iniciadas", color: "#80ffb5" },
      { label: "Profile visits", value: `${(96.7 * scale).toFixed(1)}K`, delta: "+22%", detail: "Tráfico al perfil", color: "#ff77bc" },
    ...
    reachByFormat: [
      { label: "Reels", pct: 92, color: "#58e6ff" },
      { label: "Carr.", pct: 63, color: "#d8ff63" },
      { label: "Stories", pct: 48, color: "#ff77bc" },
      { label: "Ads", pct: 36, color: "#ffc857" },
      { label: "Lives", pct: 24, color: "#80ffb5" },
    ],
    reachTotal: `${(1.42 * scale).toFixed(2)}M`,
    retention: [
      { label: "0-3s", pct: 100, color: "#58e6ff" },
      { label: "3-8s", pct: 78, color: "#80ffb5" },
      { label: "8-15s", pct: 57, color: "#d8ff63" },
      { label: "15-30s", pct: 42, color: "#ffc857" },
      { label: "Final", pct: 31, color: "#ff6f61" },
    ],
    retentionAvg: "42%",
    funnel: [
      { label: "Reach", value: `${(1.42 * scale).toFixed(2)}M`, pct: 100, color: "#58e6ff" },
      { label: "Profile visits", value: `${(96.7 * scale).toFixed(1)}K`, pct: 68, color: "#d8ff63" },
      { label: "Bio clicks", value: `${s(3772)}`, pct: 42, color: "#ffc857" },
      { label: "DM starts", value: `${s(2184)}`, pct: 31, color: "#ff77bc" },
    ...
    insights: [
      { title: "Dolor dominante", text: "La audiencia confunde volumen de posteos con sistema de demanda.", color: "#58e6ff" },
      { title: "Objeción caliente", text: "No quieren vender agresivo, pero tampoco quieren seguir invisibles.", color: "#ff6f61" },
      { title: "Frase reusable", text: "No me falta contenido, me falta que alguien me responda los DMs.", color: "#d8ff63" },
      { title: "Creencia a romper", text: "Postear más no arregla un mensaje que no hace elegir.", color: "#ff77bc" },
    ],
  };
}
...
export function emptyMetrics(accountId: string): AccountMetrics {
  ...
  return {
    ...
    engagementRate: "— ER",
    engagementMix: [
      { label: "Likes", value: "0%", color: "#58e6ff" },
      { label: "Comentarios", value: "0%", color: "#d8ff63" },
      { label: "Saves", value: "0%", color: "#ff77bc" },
      { label: "Shares", value: "0%", color: "#ffc857" },
      { label: "DMs", value: "0%", color: "#ff6f61" },
    ],
    reachByFormat: [
      { label: "Reels", pct: 0, color: "#58e6ff" },
      { label: "Carr.", pct: 0, color: "#d8ff63" },
      { label: "Stories", pct: 0, color: "#ff77bc" },
      { label: "Ads", pct: 0, color: "#ffc857" },
      { label: "Lives", pct: 0, color: "#80ffb5" },
    ],
    reachTotal: "—",
    retention: [
      { label: "0-3s", pct: 0, color: "#58e6ff" },
      { label: "3-8s", pct: 0, color: "#80ffb5" },
      { label: "8-15s", pct: 0, color: "#d8ff63" },
      { label: "15-30s", pct: 0, color: "#ffc857" },
      { label: "Final", pct: 0, color: "#ff6f61" },
    ],
    retentionAvg: "—",
    funnel: [
      { label: "Reach", value: "—", pct: 0, color: "#58e6ff" },
      { label: "Profile visits", value: "—", pct: 0, color: "#d8ff63" },
      { label: "Bio clicks", value: "—", pct: 0, color: "#ffc857" },
      { label: "DM starts", value: "—", pct: 0, color: "#ff77bc" },
...
export function buildSeed(): Db {
  const accounts = [
    { id: "acc_abrahan", name: "Abrahan Narcio", handle: "@soyabrahannarcio", kind: "propia" as const, color: "#58e6ff" },
    { id: "acc_demo", name: "Cliente Demo", handle: "@cliente.demo", kind: "cliente" as const, color: "#d8ff63" },
  ];
  const pieces: Piece[] = [];
